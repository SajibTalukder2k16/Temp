...
def normalize(img):
    ...
    for i in range(img.shape[0]):
        for j in range(img.shape[1]):
            output_image[i][j]= (img[i][j] - min_image)/ (max_img - min_image) *255
            
    return np.array(output_image, dtype = 'uint8')
...
def double_threshold(input_image, lowThresholdRatio=0.3, highThresholdRatio=0.15):
    ...
    highThreshold = int(input_image.max() * highThresholdRatio);
    lowThreshold = int(highThreshold * lowThresholdRatio);
    
    Th_image = deepcopy(input_image)
    
    weak = 100
    strong = 255
    
    Th_image[input_image >= highThreshold] = strong
    Th_image[input_image < lowThreshold] = 0
    
    weak_i, weak_j = np.where((input_image <= highThreshold) & (input_image >= lowThreshold))
    
    Th_image[weak_i, weak_j] = weak
...
def canny_edge_detection_algo(input_image):
    ...
    sobel_x_img_normalized, sobel_y_img_normalized, sobel_image, sobel_image_normalized, theta = edge_detection_operator_sobel(noise_reduced_image)
    
    # step-3: non-maximum supression
    supressed_image, supressed_image_normalized = non_max_supression(sobel_image_normalized, theta)
    
    # step-4: Double threshold
    threshold_image = double_threshold(supressed_image_normalized)
    
    # step-5: Edge Tracking by Hysteresis
    edged_image = hysteresis(threshold_image)
