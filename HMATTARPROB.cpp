#include<bits/stdc++.h>
using namespace std;
#define fileread freopen("input_mat_prob.txt","r",stdin);
#define filewrite freopen("output_mat.txt","w",stdout);
...
vector<int> foo1(double pk[],int n)
{
    ...
    cout<<"rk\tpdf\tcdf\tcdf*(l-1)\tzk"<<endl<<endl<<" ";;
    vector<int>ret(n+1);
    for(int i=1;i<=n;i++)
    {
        //cout<<i<<"\t"<<ara[i]<<"\t"<<"\t"<<pk[i]<<"\t"<<cdf[i]<<endl;
        ...
        printf("%d \t %.2f \t %.2f \t %.2f  \t %d\n ",i-1,pk[i],cdf[i],val,llround(val));
    }
    return ret;
}
int main()
{
    ...
    cout<<"Enter the no of pixels for the target: "<<endl;
    target=foo1(ara1,n);
    for(int i=1;i<=n;i++)
    {
